...
for i in range(1,5):
    thevar = str(round(forecastdata['forecast']['forecastday'][i]['day']['maxtemp_f'])) + symbol
    dayhi.append(thevar)

for i in range(1,5):
    thevar = str(round(forecastdata['forecast']['forecastday'][i]['day']['mintemp_f'])) + symbol
    daylo.append(thevar)

for i in range(1,5):
    thevar = forecastdata['forecast']['forecastday'][i]['day']['condition']['icon']
    theicon = path + "day/" + thevar.rsplit('/',1)[1]
    dayicon.append(theicon)

# Determine if there is an alert for today and send alert to a text file for alert.py script to handle.
# If there are no alerts for today, clear the text file.
...
print("${{image {} -p 0,310 -s 50x50}}${{image {} -p 95,310 -s 50x50}}${{image {} -p 190,310 -s 50x50}}${{image {} -p 285,310 -s 50x50}}".format(dayicon[0],dayicon[1],dayicon[2],dayicon[3]))
print("${{font IBM Plex Mono:size=8}}${{voffset 30}}${{goto 2}}{} / {}${{goto 98}}{} / {}${{goto 190}}{} / {}${{goto 285}}{} / {}".format(dayhi[0],daylo[0],dayhi[1],daylo[1],dayhi[2],daylo[2],dayhi[3],daylo[3]))
print("${{goto 5}}{}${{goto 105}}{}${{goto 197}}{}${{goto 290}}{}".format(result[1],result[2],result[3],result[4]))
